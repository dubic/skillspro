...
class Validations {
    companion object {
        fun notEmpty(value: String, msg: String?) {
            if (value.trim().isEmpty()) throw IllegalArgumentException(msg ?: "value cannot be empty")
        }

        fun validMail(value: String, msg: String?) {
            val regex = "^[A-Za-z0-9+_.-]+@(.+)\$"
            val pattern: Pattern = Pattern.compile(regex)
            if (!pattern.matcher(value).matches()) throw IllegalArgumentException (msg ?: "invalid email")
        }
        ...
        fun onlyAlphabets(inputString: String, msg: String?) {
            ...
            val pattern: Pattern = Pattern.compile(regex)
            if (!pattern.matcher(inputString).matches()) throw IllegalArgumentException (msg ?: "Only alphabets accepted")
        }
    }
}